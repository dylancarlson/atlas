...
/*      readinfofile()  menu level .ri .rvi routine                     */
/************************************************************************/
void readinfofile(BOOL verbose, BOOL ask)
{
    label filename;
...
/* char verbose; */
{
    ...
    {    
        ...
        {
            doCR();
            mPrintf(" %-13s %s", "3Filename:0", filename); 

            doCR();
            mPrintf(" %-13s %s (%s minutes to download)", "3Size:0",
            size, downtime);

            doCR();
            mPrintf(" %-13s %s %s", "3When:0", date, time);

            doCR();
            mPrintf(" %-13s %s", "3Uploaded By:0", uploader);

            doCR();
            mPrintf(" %-13s %s", "3Comments:0", comment); 
            doCR();
        }
        else
        {
