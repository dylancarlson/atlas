...
/* 2-19-91(JRD) Fixed to allow unlimited string lengths.  */
...
int deansi_str_cmp(char *string1, char *string2)
{
    ...
    buf1 = malloc (sizeof(char) * (strlen (string1) + 1) ) ;
    buf2 = malloc (sizeof(char) * (strlen (string2) + 1) ) ;

    if ( !(buf1 && buf2) )
	crashout ("Failed to allocate in FORMAT.C(579)\r\n") ;
...
int deansi_str_cmpi(char *string1, char *string2)
{
    ...
    if ( !(buf1 && buf2) )
	crashout ("Unable to allocate in FORMAT.C(596).\r\n") ;
